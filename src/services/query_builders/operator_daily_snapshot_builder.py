# query_builders/operator_daily_snapshot_builder.py
    ...
    def build_fetch_query(
        ...
        WITH operator_avs_counts AS (
            ...
            FROM (
                SELECT DISTINCT ON (avs_id)
                    ...
                    operator_set_id
                FROM operator_avs_registration_history oarh
                LEFT JOIN operator_sets os ON oarh.avs_id = os.avs_id
                WHERE operator_id = :operator_id
                {block_filter}
                ORDER BY avs_id, status_changed_block DESC, id DESC
            ) latest_avs_status
        ),
        operator_delegator_counts AS (
            SELECT 
            ...
            FROM (
                ...
                WHERE operator_id = :operator_id
                {block_filter.replace('status_changed_block', 'event_block') if block_filter else ''}
                ORDER BY staker_id, event_block DESC
            ) latest_delegations
        ...
        operator_pi_commission AS (
            SELECT DISTINCT ON (operator_id)
                new_bips as pi_split_bips
            FROM operator_pi_commission_bips_set_events
            WHERE operator_id = :operator_id
            {block_filter.replace('status_changed_block', 'block_number') if block_filter else ''}
            ORDER BY operator_id, block_number DESC
        )
        SELECT
            :operator_id as operator_id,
            COALESCE(odc.delegator_count, 0) as delegator_count,
            COALESCE(oac.active_avs_count, 0) as active_avs_count,
            COALESCE(oac.active_operator_set_count, 0) as active_operator_set_count,
            opc.pi_split_bips,
            COALESCE(osc.slash_event_count_to_date, 0) as slash_event_count_to_date,
            0 as operational_days,
            TRUE as is_active
        FROM operator_avs_counts oac
        CROSS JOIN operator_delegator_counts odc
        LEFT JOIN operator_slash_counts osc ON TRUE
